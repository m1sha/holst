...
test('svg-path-d MoveTo', () => {
  ...
  expect(svgPathD.currentPosition).toStrictEqual(point(10, 10))

  incoming = 'M10 10 M60 60'
  svgPathD = new SvgPathD(incoming)
  svgPathD.toPath2D(path)
  expect(svgPathD.currentPosition).toStrictEqual(point(60, 60))

  incoming = 'M10 10 m60 60'
  svgPathD = new SvgPathD(incoming)
  svgPathD.toPath2D(path)
  expect(svgPathD.currentPosition).toStrictEqual(point(70, 70))
